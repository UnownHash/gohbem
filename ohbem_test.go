...
func TestCalculateAllRanksCompact(t *testing.T) {
	...
	for ix, test := range tests {
		...
		t.Run(testName, func(t *testing.T) {
			combinations, _ := ohbem.calculateAllRanksCompact(ohbem.currentBundle(), &test.stats, test.cpCap)
...
func BenchmarkCalculateAllRanksCompact(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = ohbem.calculateAllRanksCompact(ohbem.currentBundle(), &PikachuStats, 5000)
	}
}
